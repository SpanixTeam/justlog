...
const Emote = ({
	emote,
	type,
}: { emote: IEmote; type: 'twitch' } | { emote: ThirdPartyEmote; type: 'thirdparty' }) => {
	const [hovering, setHover] = useState(false);
	if (type === 'thirdparty')
		return (
			<div
				style={{ maxHeight: 18, width: 'auto', display: 'inline-block' }}
				onMouseEnter={() => setHover(true)}
				title={emote.code}>
				<BaseEmote
					className="emote"
					alt={emote.code}
					src={hovering ? emote.urls.big : emote.urls.small}
				/>
			</div>
		);
	else
		return (
			<div
				...
				<BaseEmote
					className="emote"
					alt={emote.code}
					src={`https://static-cdn.jtvnw.net/emoticons/v2/${emote.id}/default/dark/${hovering ? '3.0' : '1.0'}`}
				/>
			</div>
		);
};
...
export function Message({
	...
	message: LogMessage;
	thirdPartyEmotes: Array<ThirdPartyEmote>;
}): JSX.Element {
...
export const CustomTooltip = styled(({ className, ...props }: TooltipProps) => (
	<Tooltip {...props} followCursor placement="right-start" classes={{ popper: className }} />
))`
